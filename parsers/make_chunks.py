...
DECLARATION = {
    ...
    "cs": ["class_declaration", "method_declaration", "function_declaration", "interface_declaration"],
    "ts": ["class_declaration", "function_declaration", "interface_declaration", "type_alias_declaration"],
    "tsx": ["class_declaration", "function_declaration", "interface_declaration", "type_alias_declaration"],
}
...
def chunk_code(code: str, extension: str) :
    ...
    def chunk(node):
        ...
        if node.type in IMPORT[extension]:
            base_chunk += "<IMPORT>" + concatenate_lines(code_lines, node.start_point, node.end_point) + "</IMPORT>\n"
            return
        elif node.type in DECLARATION[extension] or (node.type == "variable_declarator" and any([child.type == "arrow_function" for child in node.children])):
            identifier, declaration, body = chunk_declaration(code_lines, node, extension)
            ...
            base_chunk += concatenate_lines(code_lines, node.start_point, node.end_point)
        else:
...
def chunk_declaration(code_lines, node, extension):
    ...
    def chunk(node):
        ...
        if not node.children:
            declaration += concatenate_lines(code_lines, node.start_point, node.end_point)
        else:
            for child in node.children:
                chunk(child)
